...
- 30 tests in `Engineering/tests/test_arms.py` covering interleaving, per-arm models, the three cost states, partial-report fallback, substitution detection, output-length normalization, and the deliberate refusal to print a cost delta when either side's cost is unknown.
